...
const OnewayFareDetails = ({ data }) => {
  const priceRows = Array.isArray(data?.pricebreakdown)
    ? data.pricebreakdown
    : [];
  ...
  const summaryRows = [
    {
      ...
      value: formatAmount(
        data?.clientFare || data?.AirFareData?.price?.grandTotal || data?.netPrice
      ),
    },
    { label: "Discount", value: formatAmount(totalDiscount) },
    {
      label: "Agent Payable",
      value: formatAmount(
        data?.agentFare || data?.netPrice || data?.AirFareData?.price?.total
      ),
    },
  ];
