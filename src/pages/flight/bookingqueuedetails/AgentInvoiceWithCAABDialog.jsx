...
const AgentInvoiceWithCAABDialog = ({
  open,
  onClose,
  onDownloadWithPrice,
  ...
  const initialAgency = defaultAgencyName ?? agentData?.agentName ?? agentData?.name ?? agentData?.companyName ?? "TRIP FINDY";
  const initialCAAB = defaultCivilAviationNumber ?? agentData?.caabNumber ?? agentData?.civilAviationNumber ?? "0014537";
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: 2.5 }}>
          ...
          <Box sx={{ display: "flex", flexDirection: "column", gap: 0.5, mt: 0.5 }}>
            <Box
              ...
              sx={optionSx}
            >
              <FileDownloadIcon sx={{ fontSize: 18, color: "var(--primary-color)" }} />
              Download with price
            </Box>
            <Box
              component="button"
...
export default AgentInvoiceWithCAABDialog;
