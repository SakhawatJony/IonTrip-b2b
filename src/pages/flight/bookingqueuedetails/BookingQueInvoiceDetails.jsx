            ...
            {travellers.map((pax, idx) => (
              <TableRow key={idx}>
                ...
                <TableCell sx={{ fontSize: 11 }}>
                  {Array.isArray(pax?.Ticket) && pax.Ticket.length > 0
                    ? pax.Ticket.map((t) => t?.TicketNo).filter(Boolean).join(", ") || data?.airlinePNR || data?.gdsPNR || "N/A"
                    : data?.airlinePNR || data?.gdsPNR || "N/A"}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
